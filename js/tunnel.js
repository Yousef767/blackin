...
let textInner = document.querySelector('.firstText');
tl.to(tubePerc, {
  percent: .96,
  ...
  onUpdate: function () {
    ...
    var scrollProgress = tl.progress();
    logo.style.opacity = 1 - (scrollProgress*3);
    if(scrollProgress >= .4){
      text.style.opacity =  (scrollProgress - .1) >= .6 ? 1 : (scrollProgress - .1);
    }
    if(scrollProgress <= .4){
...
// Retrieve the scroll progress within the tunnel
